...
public class ConfigurationClientTest {
    ...
    public void getSetting() throws Exception {
        ...
        setting.key("hello");
        Call<ConfigurationSetting> c = client.getSetting(setting);
        Response<ConfigurationSetting> response = c.execute();
        assertNotNull(response);
        assertEquals(200, response.code());
        assertNotNull(response.body());
        assertEquals("hello", response.body().key());
        assertEquals("world", response.body().value());
    }
